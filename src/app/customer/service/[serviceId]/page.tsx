...
const getSession = () => {
  try {
    ...
    if (userRaw) {
      ...
      return { userId: user?.userId || user?.id || user?._id || undefined, token };
    }
    // fallback keys
    const userId = localStorage.getItem("userId") || localStorage.getItem("customerId") || undefined;
    return { userId, token };
  } catch {
...
export default function ServiceDetailsPage({ params }: { params: Promise<{ serviceId: string }> }) {
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      try {
        ...
        const response = await fetch(`${BASE_URL}/service/get-details`, {
          method: "POST",
          headers: {
        ...
        if (result.success && result.data) {
          ...
          const transformedService: Service = {
            ...
            rating: apiService.rating ?? 4.5,
            totalReviews: apiService.totalReviews ?? 0,
            bookingCount: apiService.bookingCount ?? 0,
            packages:
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            location: {
              ...
              province: apiService.state || apiService.province || "",
  ...
  const saveService = async (status: "INTERESTED" | "PENDING") => {
    if (!service) return;
    ...
    return (
      <CustomerMainLayout>
        <div className="max-w-4xl mx-auto px-4 py-8">
          <Link href="/customer/service/browse" className="inline-flex items-center text-sm text-purple-700 hover:text-purple-900">
      ...
      <CustomerMainLayout>
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-8 text-center">
            ...
            <p className="text-gray-600 mb-6 text-sm">{error || "Return to browse and select another service."}</p>
            <Link href="/customer/service/browse" className="px-5 py-2 rounded-lg bg-purple-600 text-white text-sm font-medium hover:bg-purple-700">
              Browse Services
            </Link>
          </div>
        </div>
  ...
  const startingPrice =
    service.packages.length > 0 ? Math.min(...service.packages.map((p) => parseFloat(p.price) || 0)) : 0;
        ...
        <div className="bg-white rounded-2xl shadow border border-gray-200 overflow-hidden">
          {service.photos.length > 0 && (
            <div className="relative h-72 w-full">
              <img src={service.photos[0]} alt={service.serviceName} className="h-full w-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-black/10 to-transparent" />
              <div className="absolute bottom-4 left-6 text-white">
                <h1 className="text-3xl font-bold">{service.serviceName}</h1>
                <div className="mt-2 flex flex-wrap gap-3 text-xs">
                  <span className="bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm">{service.category}</span>
                  ...
                  {service.packages.map((pkg, index) => (
                    <div key={index} className="border border-purple-200 rounded-lg p-4 bg-purple-50">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="text-lg font-bold text-purple-600">LKR {parseInt(pkg.price).toLocaleString()}</div>
                      </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {service.photos.slice(1).map((photo, index) => (
                    <div key={index} className="relative group rounded-lg overflow-hidden">
                      <img src={photo} alt={`${service.serviceName} gallery image ${index + 1}`} className="h-40 w-full object-cover group-hover:scale-105 transition-transform" loading="lazy" />
            ...
            <div className="pt-6 border-t border-gray-100 flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={saving}
                  className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${saving ? "bg-gray-400 text-white cursor-not-allowed" : "bg-purple-600 text-white hover:bg-purple-700"}`}
                >
                ...
                <button
                  ...
                  disabled={saving}
                  className={`px-5 py-2 rounded-lg border text-sm font-medium transition-colors ${saving ? "border-gray-300 text-gray-400 cursor-not-allowed" : "border-purple-300 text-purple-700 hover:bg-purple-50"}`}
                >
